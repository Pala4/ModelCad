...
switch(Input->ID_ELEM)  //передача сигнала от входного объекта
 {
...
CEls::CEls(void)
{
   ...
   Ls = new TList;
	...
	Chislo_Up_Svjaz=0;
        Chislo_Dop_Svjaz=0;
        X_input=false, Y_output=false; //нужны чтобы один раз вводился X и Y
	Dop_Svz=NULL;
	Error=NULL; // для ошибки закольцованности сумматора
...
void CEls::AddLink(CElement *InEl, CElement *OutEl, T_ORIENT IN_OR, T_ORIENT OUT_OR)
{
	...
	switch(InEl->ID_ELEM)
   {
   	case ID_STDELEMENT:
         ...
         {
         	if(Application->MessageBox("Ошибка: связь уже существует!\nЗаменить её?",
                                       "Предупреждение",
      ...
      break;
      case ID_OUT:
      	Application->MessageBoxA("У объекта отсутствует выход!", "Ошибка",
                                  MB_OK | MB_ICONERROR);
         delete L;
         return;
   }
   ...
   switch(OutEl->ID_ELEM)
   {
      ...
      break;
      case ID_IN:
      	Application->MessageBoxA("У объекта отсутствует вход!", "Ошибка",
                                  MB_OK | MB_ICONERROR);
         delete L;
         switch(InEl->ID_ELEM)
         {
...
void __fastcall CEls::Linking(CElement *El, T_ORIENT OR)
{
   ...
   else
   {
   	if(InElLnk == El)
      {
      	Application->MessageBox("Ошибка: недопустимо связывание объекта с самим собой!",
...
//Memo1->Lines->Add(ind_Up[0]);
 ...
 {
 /*
 if(!X_input) ff->Memo1->Lines->Add("oтсутствует X");
  if(!Y_output) ff->Memo1->Lines->Add("oтсутствует Y");
  */
 }
}
//---------------------------------------------------------------------------
...
int TRR=Kol_vo;
//ff->Memo1->Lines->Add("Кол-во" + IntToStr(TRR));
...
switch(Dvigenie->ID_ELEM)
 ...
 case ID_STDELEMENT:
         if(((CStdElement*)Dvigenie)->Output!=NULL)
         {
           ...
           {istina=false;
           //ff->Memo1->Lines->Add("Закольцованность");
           }
           else
 ...
 if(Up_Svjaz_Naidena)
 {
 	//ff->Memo1->Lines->Add("Главная связь");
...
// Dop_Svz[index_Up_svz]=new Node_DopMas;
 for(int i=0;i<index_Up; i++) //просмотр гл связи
  {
  if(Error_NULL_Svjaz) break;
  if(Up_Svjaz[i]->ID_ELEM==ID_UZEL)  //начало определения побочных связей
   {
    Vspom_Massiv[0]=Up_Svjaz[i];
    if ((((CUzel*)Up_Svjaz[i])->Output)==Up_Svjaz[i+1])// начало движ по побочной связи
     {
...
if(!Error_NULL_Svjaz)
{
//Рекурсивный цикл опрделяющий связи
while(istin2)
 {
...
//             Kol_voDop_Sv++;
    ...
    case ID_SUMMATOR1:
          {
           bool Sum_Naiden=false; // для определения обратного сумматора в доп связи
           for(int j=0;j<index_mas-1; j++)
